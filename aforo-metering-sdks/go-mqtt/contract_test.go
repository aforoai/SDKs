...
func emit(b *Billing, opts ...EventOptions) {
	b.RecordPublish("cust_1", "client-1", "devices/1/telemetry", 1, false, 42, opts...)
}
...
func TestContractDefaultProductType(t *testing.T) {
	...
	if got := srv.events()[0]["productType"]; got != DefaultProductType || DefaultProductType != "MQTT_BROKER" {
		t.Errorf("productType = %v, want MQTT_BROKER", got)
	}
