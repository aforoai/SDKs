...
func emit(b *Billing, opts ...EventOptions) {
	...
	b.Record(ctx, "GetUser", "UNARY", 1, nil, 3, opts...)
}

func shutdown(b *Billing) error { return b.Shutdown(context.Background()) }
...
func TestContractDefaultProductType(t *testing.T) {
	...
	if got := srv.events()[0]["productType"]; got != DefaultProductType || DefaultProductType != "GRPC_API" {
		t.Errorf("productType = %v, want GRPC_API", got)
	}
