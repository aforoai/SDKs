...
### Added
- `Builder.productType(String)` (default `MQTT_BROKER`) and `getProductType()`: the top-level `productType` the ingestor requires, previously hard-coded. Trimmed and uppercased; unknown values are passed through.
