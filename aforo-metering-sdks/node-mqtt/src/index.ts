...
export class AforoMqttBilling {
  ...
  private push(
    partial: Omit<MqttUsageEvent, 'metricName' | 'quantity' | 'occurredAt' | 'idempotencyKey' | 'productType'> & { productType?: string }
  ): void {
    ...
    if (!mqttTopic || !mqttTopic.trim()) return; // mqttTopic is required on every MQTT_BROKER event
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (!isRetryableStatus(res.status)) {
          ...
          this.onError(new Error(`MQTT metering batch rejected with HTTP ${res.status}${details ? ` — ${details}` : ''} (dropped ${batch.length} events, not retried)`));
          return;
        }
