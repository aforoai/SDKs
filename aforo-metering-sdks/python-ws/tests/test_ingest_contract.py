...
ALLOWED_TOP_LEVEL = {
    ...
    "wsConnectionId", "wsDirection", "wsFrameType", "wsCloseReason",
}
...
def _emit_customer(b, customer_id):
    b.push({"customerId": customer_id, "wsConnectionId": "conn-1",
            "wsDirection": "CLIENT_TO_SERVER", "wsFrameType": "CLOSE",
            "wsCloseReason": "NORMAL_CLOSURE", "messageCount": 4, "dataBytes": 99,
            "durationMs": 1500})
...
def test_posts_batch_to_v1_ingest_batch_with_dto_shape(monkeypatch):
    ...
    assert ev["wsConnectionId"] == "conn-1"
    assert ev["wsDirection"] == "CLIENT_TO_SERVER"
    assert ev["wsFrameType"] == "CLOSE"
    assert ev["messageCount"] == 4
    assert ev["dataBytes"] == 99
    assert ev["executionDurationMs"] == 1500  # legacy durationMs input mapped
