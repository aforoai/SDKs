...
from aforo_ws_metering.client import AforoWsBilling, track_websockets_connection
...
def test_default_product_type(env):
    ...
    _emit(b)
    b.shutdown()
    assert [e["productType"] for e in _events(h)] == ['WEBSOCKET_API']
    assert errors == []
...
def test_blank_client_product_type_uses_package_default(env):
    ...
    _emit(b)
    b.shutdown()
    assert _events(h)[0]["productType"] == 'WEBSOCKET_API'
...
def _make(errors, **kw):
    return AforoWsBilling(tenant_id="t", product_id="p", api_key="k",
                          ingestor_url="https://api.aforo.ai", flush_count=10_000,
                          flush_interval_sec=3600, on_error=errors.append, **kw)


def _emit(b, product_type=None):
    ev = {"customerId": "c1", "wsConnectionId": "conn-1"}
    if product_type is not None:
        ev["productType"] = product_type
    b.push(ev)


def test_missing_ws_connection_id_is_dropped(env):
    h, _s, errors = env
    b = _make(errors)
    b.push({"customerId": "c1"})
    b.shutdown()
    assert h.requests == []


def test_tracker_passes_product_type_through(env):
    import asyncio
    ...
    async def run():
        async with await track_websockets_connection(b, _Ws(), "c1", product_type="api"):
            pass
    ...
    events = _events(h)
    assert len(events) == 2
    assert {e["productType"] for e in events} == {"API"}
    assert all(e["wsConnectionId"] for e in events)
