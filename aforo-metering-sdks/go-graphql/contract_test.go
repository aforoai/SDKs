...
func emit(b *Billing, opts ...EventOptions) {
	b.Record("cust_1", "mutation CreateUser { createUser { id } }", "", 3, false, opts...)
}
...
func TestContractDefaultProductType(t *testing.T) {
	...
	if got := srv.events()[0]["productType"]; got != DefaultProductType || DefaultProductType != "GRAPHQL_API" {
		t.Errorf("productType = %v, want GRAPHQL_API", got)
	}
