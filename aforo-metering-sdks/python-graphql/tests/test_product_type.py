...
from aforo_graphql_metering.client import AforoGraphQlBilling
...
def test_default_product_type(env):
    ...
    _emit(b)
    b.shutdown()
    assert [e["productType"] for e in _events(h)] == ['GRAPHQL_API']
    assert errors == []
...
def test_blank_client_product_type_uses_package_default(env):
    ...
    _emit(b)
    b.shutdown()
    assert _events(h)[0]["productType"] == 'GRAPHQL_API'
...
def _make(errors, **kw):
    return AforoGraphQlBilling(tenant_id="t", product_id="p", api_key="k",
                               ingestor_url="https://api.aforo.ai", flush_count=10_000,
                               flush_interval_sec=3600, on_error=errors.append, **kw)


def _emit(b, **kw):
    b.record(customer_id="c1", query="mutation M { a }", operation_name=None,
             duration_ms=1, has_errors=False, **kw)


def test_gql_operation_type_always_populated(env):
    h, _s, errors = env
    b = _make(errors)
    _emit(b)
    b.record(customer_id="c1", query="subscription S { a }", operation_name=None,
             duration_ms=1, has_errors=False)
    b.record(customer_id="c1", query="{ a }", operation_name=None, duration_ms=1, has_errors=False)
    b.shutdown()
    assert [e["gqlOperationType"] for e in _events(h)] == ["MUTATION", "SUBSCRIPTION", "QUERY"]
