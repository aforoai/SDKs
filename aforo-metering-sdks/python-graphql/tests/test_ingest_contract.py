...
ALLOWED_TOP_LEVEL = {
    ...
    "gqlOperationType", "gqlOperationName", "gqlComplexity", "gqlFieldCount", "gqlHasErrors",
}
...
def _emit_customer(b, customer_id):
    b.record(customer_id=customer_id, query="query GetUser { user { id name } }",
             operation_name="GetUser", duration_ms=12, has_errors=False, response_bytes=64)
...
def test_posts_batch_to_v1_ingest_batch_with_dto_shape(monkeypatch):
    ...
    assert ev["executionDurationMs"] == 12
    assert ev["dataBytes"] == 64
