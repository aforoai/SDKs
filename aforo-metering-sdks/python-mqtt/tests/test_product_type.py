...
def test_default_product_type(env):
    ...
    _emit(b)
    b.shutdown()
    assert [e["productType"] for e in _events(h)] == ['MQTT_BROKER']
    assert errors == []
...
def test_blank_client_product_type_uses_package_default(env):
    ...
    _emit(b)
    b.shutdown()
    assert _events(h)[0]["productType"] == 'MQTT_BROKER'
...
def _make(errors, **kw):
    return AforoMqttBilling(tenant_id="t", product_id="p", api_key="k",
                            ingestor_url="https://api.aforo.ai", flush_count=10_000,
                            flush_interval_sec=3600, on_error=errors.append, **kw)


def _emit(b, **kw):
    b.push(customer_id="c1", topic="a/b", qos=0, retained=False,
           event_type="PUBLISH", client_id="dev-1", **kw)
