...
aforo_metering {
  ...
  ingestor_url = "https://api.aforo.ai/v1/ingest/batch"
}
```
...
| `ingestor_url` | string | `https://api.aforo.ai/v1/ingest/batch` | The ingestor batch endpoint. Override per environment; keep the `/v1/ingest/batch` path, which is the only one that accepts the `{"events": [...]}` body the plugin sends. |
